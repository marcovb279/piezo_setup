...
int main(int argc, char *argv[]) {
  ...
  // IFM_OPTION_DEBUGFILES let the DLL create files with information for debug
  // purposes
  ...
  // search for connected devices; returns the number of via USB connected
  // devices
  // int cnt;
  ...
  // Alternatively, if the COM-Interface is known the device can also be opened
  // directly Windows: devNo=IfmOpenCom(1); for COM1 Unix:
  // devNo=IfmOpenCom("/dev/ttyACM0"); for the first device

  // IfmOpenUSB has returned a number (devNo) which describes the device in
  // further calls to the DLL.
  ...
  // in some applications IFM_MEAS_FILTER_NONE may be required for unfiltered
  // output

  // changed: 3 channels + samplecounter

  error = IfmSetMeasurement(
      devNo, IFM_MEAS_THREECHANNEL | IFM_MEAS_LENGTH | IFM_MEAS_FILTER_DEFAULT,
      0.5);
  if (error > 0) {
    printf("Error during opening the device.\n");
    IfmClose();
  ...
  // thats why before requesting things from the configuration, wait until it's
  // available
  ...
  if (IfmAngleAvailable(devNo, IFM_CHANNEL1 | IFM_CHANNEL2))
    ...
    printf("Angle calculation between channels 2 and 1 is NOT possible.\n");
  ...
  if (IfmAngleAvailable(devNo, IFM_CHANNEL3 | IFM_CHANNEL2))
    ...
    printf("Angle calculation between channels 2 and 3 is NOT possible.\n");

  // Set the length values to zero; assuming the measurement mirror is at the
  ...
  while (!kbhit()) {
    ...
    if (IfmValueCount(devNo)) {
      ...
      // this is necessary to access the same syncronuously sampled values (e.g.
      // different channels) at different times IfmValueCount is decremented
      IfmGetValues(devNo);
      // get the values together with the angles
      printf("%8.0lf �m %8.0lf nm %8.0lf nm - yaw (2-1) %.3f pitch (2-3) %.3f  "
             "    \r",
